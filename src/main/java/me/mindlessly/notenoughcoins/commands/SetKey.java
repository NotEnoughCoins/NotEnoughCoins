...
public class SetKey extends CommandBase {
	...
	public void processCommand(ICommandSender sender, String[] args)
	{
		if(args.length > 0) {
			...
			ChatComponentText runtext = new ChatComponentText(EnumChatFormatting.GOLD+("NEC ") + EnumChatFormatting.GREEN+("API Key set to "+ args[0]));
			sender.addChatMessage(runtext);
		}
		else {
			ChatComponentText error  = new ChatComponentText(EnumChatFormatting.GOLD+("NEC ") + EnumChatFormatting.RED+("Usage: /setkey <key>"));
