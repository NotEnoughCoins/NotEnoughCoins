...
public class ApiHandler {
    ...
    public static void getBins(HashMap<String, Double> dataset) {
        ...
        try {
            JsonObject binJson = Objects.requireNonNull(getJson("https://moulberry.codes/lowestbin.json")).getAsJsonObject();
    ...
    public static void updatePurseCoins() {
        try {
            JsonArray profilesArray = Objects
                    .requireNonNull(getJson("https://api.hypixel.net/skyblock/profiles?key=" + Config.apiKey + "&uuid=" + UUID))
                                        ...
                                        if (startingBid < entry.getValue()) {
                                            if (startingBid <= Toggle.purse) {
                                                ...
                                                if (entry.getValue() - startingBid > Config.minProfit) {
                                                    ...
                                                    if (profit > Config.minProfit && percentProfit > Config.minProfitPercentage) {
